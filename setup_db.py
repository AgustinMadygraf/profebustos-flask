...
if __name__ == "__main__":
    ...
    table_creator.create_conversiones_table()
    table_creator.create_etiquetas_table()
    checker = MySQLSetupChecker()
    checker.logger.info("=== Verificación de entorno MySQL ===")
    if checker.connect():
        if checker.check_database_exists():
            checker.check_table_exists('conversiones')
            checker.check_table_exists('etiquetas')
    checker.close()
